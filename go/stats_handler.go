...
func getUserStatisticsHandler(c echo.Context) error {
	...
	// ランク算出
	var userRankings []UserRankingEntry
	if err := tx.SelectContext(ctx, &userRankings, "SELECT u.name AS user_name, (COUNT(r.id) + IFNULL(SUM(l.tip), 0)) AS score FROM users AS u LEFT JOIN reactions r ON r.user_id = u.id LEFT JOIN livecomments l ON l.user_id = u.id GROUP BY u.name"); err != nil {
	...
	// 	})
	// }
	ranking := UserRanking(userRankings)
	sort.Sort(ranking)
...
func getLivestreamStatisticsHandler(c echo.Context) error {
	...
	var livestreamRankingEntries []LivestreamRankingEntry
	query := `
		SELECT
			ls.id,
		...
		LEFT JOIN reactions r
			ON r.livestream_id = ls.id
		LEFT JOIN livecomments lc
			ON lc.livestream_id = ls.id
		GROUP BY ls.id
	`
	if err := dbConn.SelectContext(ctx, &livestreamRankingEntries, query); err != nil && !errors.Is(err, sql.ErrNoRows) {
		if errors.Is(err, sql.ErrNoRows) {
	...
	for _, entry := range livestreamRankingEntries {
